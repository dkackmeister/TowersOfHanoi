...
def menu_screen():  # to be called before starting actual game loop
    global screen, n_disks, game_done, minimum_steps
    menu_done = False
    while not menu_done:
        screen.fill(white)
        ...
        blit_text(screen, 'Use arrow keys to select difficulty:', (320, 220), font_name='sans serif', size=30,
                  color=black)
        if n_disks <= 3:
            blit_text(screen, str(n_disks) + ' Easy', (325, 261), font_name='sans serif', size=40, color=black)
            blit_text(screen, str(n_disks) + ' Easy', (324, 260), font_name='sans serif', size=40, color=green)
        elif 4 <= n_disks < 7:
            blit_text(screen, str(n_disks) + ' Medium', (325, 261), font_name='sans serif', size=40, color=black)
            blit_text(screen, str(n_disks) + ' Medium', (324, 260), font_name='sans serif', size=40, color=gold)
        elif n_disks >= 7:
            blit_text(screen, str(n_disks) + ' Hard', (325, 261), font_name='sans serif', size=40, color=black)
            blit_text(screen, str(n_disks) + ' Hard', (324, 260), font_name='sans serif', size=40, color=red)
...
def main():
    ...
    while not game_done:
        ...
        if steps <= minimum_steps:
            blit_text(screen, f'Steps: {str(steps)}', (320, 20), font_name='sans seriff', size=50, color=black)
        elif steps > minimum_steps:
            blit_text(screen, f'Steps: {str(steps)}', (320, 20), font_name='sans seriff', size=50, color=red)

        blit_text(screen, f'Minimum Possible Steps: {str(minimum_steps)}', (320, 55), font_name='san seriff', size=50, color=black)
        pygame.display.flip()
        if not floating:
            check_win_state()
